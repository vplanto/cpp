...
1. Предмет 1: вага = 2, цінність = 3
2. Предмет 2: вага = 3, цінність = 4
3. Предмет 3: вага = 4, цінність = 5
...
| **1** (тільки предмет 1: в=2, ц=3) | 0 | 0 | **3** | 3 | 3 | 3 |
| **2** (предмети 1, 2: в=3, ц=4) | 0 | 0 | 3 | 4 | 4 | **7** |
| **3** (усі три предмети: в=4, ц=5) | 0 | 0 | 3 | 4 | 5 | **7** |

**Як обчислюється клітинка `dp[2][5]` (додаємо предмет 2 при ліміті ваги 5):**
Формула: `dp[2][5] = max(dp[1][5], dp[1][5 - 3] + 4)`

1. **Варіант 1 (НЕ беремо предмет 2):** Дивимося на значення безпосередньо над поточною клітинкою (`dp[1][5]`). Отримуємо **3**.
2. **Варіант 2 (Беремо предмет 2):** Забираємо під нього вагу `w_2 = 3` (залишається `5 - 3 = 2`). Дивимося в попередньому рядку найкращу цінність для вільної ваги 2 (`dp[1][2] = 3`) і додаємо до неї цінність поточного предмета (`v_2 = 4`). Отримуємо `3 + 4 =` **7**.
