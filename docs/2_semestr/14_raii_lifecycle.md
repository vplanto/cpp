...
> **📛 Найгірша назва в історії C++**
>
> Б'ярн Страуструп придумав цю ідіому у 80-х і назвав її **RAII** — *Resource Acquisition Is Initialization*. І сам же пізніше визнав, що це **жахлива назва**.
>
> Проблема в тому, що назва акцентує увагу на *захопленні* ресурсу (конструкторі), тоді як справжня магія — це *автоматичне звільнення* (деструктор). Читаючи «RAII», студенти думають про ініціалізацію, а не про гарантоване прибирання.
>
> У 2001 році Peter Sommerlad та Marc Rüdiger запропонували точнішу назву: **SBRM** — *Scope-Bound Resource Management* («Керування ресурсами, прив'язане до області видимості»). Вона одразу пояснює суть: ресурс живе рівно стільки, скільки живе `scope` (блок `{}`).
>
> Страуструп погодився, що SBRM краще. Але RAII вже встигла стати частиною мови програмістів по всьому світу — в книгах, статтях, інтерв'ю. Перейменувати її тепер — як перейменувати вулицю: технічно можна, але всі ще довго будуть плутатись.
>
> **Висновок для нас:** Погана назва — це технічний борг. Іноді він переживає самого автора.
...
Після теоретичного ознайомлення рекомендуємо розібрати наступний робочий приклад:
* 👨‍💻 **[09-oop-intro/encapsulation.cpp](https://github.com/vplanto/cpp/blob/main/09-oop-intro/encapsulation.cpp)** — Демонстрація того, як повернення посилання на приватний член може порушувати інкапсуляцію, і як `const_cast` може обійти захист. Використовує `this` як повернення `*this` у методах.
