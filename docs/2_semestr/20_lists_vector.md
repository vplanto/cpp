# Лекція 20: Лінійні структури — Битва Vector vs List

[← Лекція 19](19_complexity_profiling.md) | [Index](index.md) | [Далі: Лекція 21 →](21_sorting_algorithms.md)



## Мета
Зрозуміти, як реалізовані списки "під капотом". Навчитися обирати між масивом (`std::vector`) та зв'язним списком (`std::list`).
Спойлер: У 95% випадків ви маєте брати Vector.

## Експрес-опитування
1.  Якщо ми додаємо елемент у **середину** масиву, що стається з іншими елементами?
2.  Чи потрібно зсувати елементи при додаванні в середину зв'язного списку?
3.  Скільки пам'яті (байт) займає зберігання одного `int` (4 байти) у двозв'язному списку на 64-бітній системі?

<details markdown="1">
<summary>Інженерна відповідь</summary>

1.  Ми мусимо **зсунути** (скопіювати) всі наступні елементи вправо. Це повільно $O(N)$.
2.  **Ні.** Ми просто змінюємо вказівники у сусідів. Це швидко $O(1)$.
3.  **24 байти!** (4 байти значення + 4 padding + 8 вказівник `next` + 8 вказівник `prev`). Оверхед у 5 разів!

</details>

---

## Частина 1: Абстракція "Список" (ADT)

[cite_start][cite: 4]
Список — це упорядкована колекція елементів. Ми хочемо робити:
* `insert(pos, value)`
* `remove(pos)`
* `get(index)`

Але **реалізація** диктує швидкість.

---

## Частина 2: Dynamic Array (`std::vector`)

[cite_start]Це аналог `ArrayList` у Java[cite: 4].
**Суть:** Суцільний блок пам'яті. Якщо місце закінчилось — виділяємо новий блок (x2), копіюємо все туди, старий видаляємо.

**Переваги:**
* **Pros:** **Cache Friendly:** Всі дані лежать поруч. Процесор "ковтає" їх пачками (Cache Lines).
* **Pros:** **Random Access:** `vec[500]` — це миттєва математика адреси ($Base + 500 \cdot Size$). $O(1)$.

**Недоліки:**
* **Cons:** **Insert/Delete in middle:** Треба зсувати хвіст. [cite_start]$O(N)$[cite: 4].
* **Cons:** **Reallocation:** Періодичні дорогі копіювання при зростанні.

---

## Частина 3: Linked List (`std::list`)

[cite_start]Це класичний двозв'язний список (Double Linked List)[cite: 4].
**Суть:** Кожен елемент — це окремий об'єкт (Node) у купі (Heap), який знає, хто його сусіди.



```cpp
struct Node {
    int value;
    Node* next; // +8 байт
    Node* prev; // +8 байт
};

```

**Переваги:**

* **Pros:** **Insert/Delete in middle:** Просто перекинути вказівники.  (якщо у вас вже є ітератор на позицію).


* **Pros:** **No Reallocation:** Вказівники залишаються валідними завжди.

**Недоліки:**

* **Cons:** **Cache Killer:** Вузли розкидані по пам'яті хаотично. Кожен перехід `node->next` — це потенційний Cache Miss.
* **Cons:** **No Random Access:** Щоб дістати 500-й елемент, треба пройти 499 попередніх. .


* **Cons:** **Memory Overhead:** Платимо за вказівники (x3 пам'яті для `int`).

---

## Частина 4: Порівняння Складності (Cheat Sheet)

На основі вашого матеріалу:

| Операція | Vector (`std::vector`) | List (`std::list`) |
| --- | --- | --- |
| **Access `[i]**` | **** (Миттєво) |  (Дуже довго) |
| **Insert Head** |  (Зсув) | **** |
| **Insert Tail** |  (Amortized) | **** |
| **Insert Middle** |  | **** (якщо є ітератор) |
| **Cache** | 🔥 Hot | 🧊 Cold |

---

## Частина 5: Вердикт Б'ярна Страуструпа

Сам творець C++ проводив тест: генерував випадкові числа і вставляв їх у відсортований список/вектор.
Теоретично `list` мав виграти (вставка в середину).
Практично **`vector` переміг**, навіть з урахуванням зсувів.

**Чому?**
Поки `list` шукає місце в пам'яті (стрибає по вказівниках), `vector` встигає зсунути мегабайт даних завдяки кешу та SIMD-інструкціям процесора.

---

## Додаток: Circular Queue (Ring Buffer)

> **💡 Visual Schema from Source Material**  
> Джерело: Stacks&Queues.pdf

![Queue — принцип FIFO та базові операції](attachments/stacks-001.jpg)

*Рис. 1: Черга (Queue) — наочне зображення FIFO принципу*

![Circular Queue — Ring Buffer](attachments/stacks-002.jpg)

*Рис. 2: Circular Queue (Ring Buffer) — оптимальне використання пам'яті через зациклений індекс*

**Проблема:** Наївна реалізація черги на масиві марнує простір.

**Наївна черга:**
```
[  ][  ][A][B][C][ ][ ]
        ↑       ↑
      front    rear
```

Після `dequeue()` двічі:
```
[  ][  ][  ][  ][C][ ][ ]
                ↑   ↑
              front rear
```

Попередній простір (`[0], [1]`) втрачено назавжди! Rear може дійти до кінця масиву, навіть якщо є вільний простір на початку.

**Рішення: Circular Queue (Ring Buffer)**

Уявляємо масив як **кільце** (wheel). Індекси "зациклюються" через модуло арифметику:

```
Візуалізація кільця:
   
      [0]
   [6]   [1]
[5]   ⚫   [2]
   [4]   [3]
   
front = 2, rear = 5
Елементи: [2], [3], [4], [5]

Після enqueue(X):
rear = (rear + 1) % 7 = 6
```

**Реалізація:**

```cpp
class CircularQueue {
    int* data;
    int front, rear, size, capacity;
    
public:
    CircularQueue(int cap) {
        capacity = cap;
        data = new int[capacity];
        front = rear = -1;
        size = 0;
    }
    
    bool enqueue(int value) {
        if (size == capacity) return false;  // Full
        
        if (front == -1) front = 0;  // First element
        
        rear = (rear + 1) % capacity;  // ⭐ Wrap around!
        data[rear] = value;
        size++;
        return true;
    }
    
    int dequeue() {
        if (size == 0) return -1;  // Empty
        
        int value = data[front];
        front = (front + 1) % capacity;  // ⭐ Wrap around!
        size--;
        
        if (size == 0) {  // Queue became empty
            front = rear = -1;
        }
        
        return value;
    }
};
```

**Index Wrapping Math:**
- `(rear + 1) % capacity` — наступний індекс після `rear`
- Якщо `rear = 6` і `capacity = 7`, то `(6 + 1) % 7 = 0` (повертаємося на початок)

**Використання:**
- Audio/Video buffers (streaming)
- Producer-Consumer patterns
- Kernel ring buffers (Linux `dmesg`)

**Ефективність:**
- **Space:** Використовується 100% масиву (немає втраченого простору)
- **Time:** O(1) для enqueue/dequeue

> **Engineering Rule:** Використовуйте `std::vector` як основу, але якщо queue pattern треба часто вставляти і видаляти з обох кінців → circular buffer ефективніший за naive queue

> **Engineering Rule:** За замовчуванням використовуйте `std::vector`.
> Використовуйте `std::list`, ТІЛЬКИ якщо вам треба часто вставляти/видаляти елементи з середини І ви не проходите по списку часто.

---

---

## Практичне застосування

**Див.:** [Практикум 13: Algorithms & Good Taste](p13_algorithms_and_taste.md) — алгоритмічний дизайн та робота зі структурами даних.

## Контрольні питання

1. У вас є список з 1 млн куль (об'єктів гри). Вам треба кожен кадр проходити по всім і оновлювати їх позицію. Що ви оберете: `vector` чи `list`?
2. Чому `std::vector::push_back` називають "амортизованим "?
3. Як перетворити `vector` на "C-style array" (вказівник на перший елемент)? (Підказка: `vec.data()`).
