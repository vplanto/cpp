...
| Категорія | Момент | Суть |
| --- | --- | --- |
| Перевага | **Cache Friendly** | Всі дані лежать поруч. Процесор "ковтає" їх пачками (**Cache Lines**). |
| Перевага | **Random Access** | `vec[500]` — миттєва адресація ($Base + 500 \cdot Size$), $O(1)$. |
| Недолік | **Insert/Delete in middle** | Треба зсувати хвіст, $O(N)$. |
| Недолік | **Reallocation** | Періодичні дорогі копіювання при зростанні. |
...
| Алгоритм (Операція) | Швидкість |
| --- | --- |
| Прямий доступ (`vec[i]`) | $O(1)$ за звернення + кеш-локальність |
| Додавання в кінець | $O(1)$, амортизовано |
| Вставка в середину | $O(N)$ через зсув |

> **💡 Чому це важливо?** Навіть якщо два алгоритми мають однакову складність $O(N)$, той, що працює з кешем (як вектор), буде в **10–100 разів швидшим** за той, що постійно спричиняє **Cache Miss** (як список), бо йому не треба кожного разу чекати на дані з повільної оперативної пам'яті (RAM).
...
| Категорія | Момент | Суть |
| --- | --- | --- |
| Перевага | **Insert/Delete in middle** | Перекинути вказівники (якщо вже є ітератор на позицію), $O(1)$. |
| Перевага | **No Reallocation** | Вказівники залишаються валідними завжди. |
| Недолік | **Cache Killer** | Вузли розкидані по пам'яті хаотично. Кожен перехід `node->next` — потенційний **Cache Miss**. |
| Недолік | **No Random Access** | Щоб дістати 500-й елемент, треба пройти 499 попередніх, $O(N)$. |
| Недолік | **Memory Overhead** | Платимо за вказівники (×3 пам'яті для `int`). |
...
| Алгоритм (Операція) | Швидкість |
| --- | --- |
| Пошук за номером | $O(N)$ через "стрибки" |
| Вставка (з ітератором) | $O(1)$ тільки вказівники |
| Послідовний обхід | $O(N)$ за обхід + Cache Miss |
...
| **Access `[i]`** | $O(1)$ | $O(N)$ |
| **Insert Head** | $O(N)$ | $O(1)$ |
| **Insert Tail** | $O(1)$, амортизовано | $O(1)$ |
...
| --- | --- |
| `enqueue` / `dequeue` | $O(1)$ |
| `dequeue` зі зсувом | $O(N)$ (наївно) |
| Додавання при "дрейфі" | **Overflow** (хоча місце є) |
...
```mermaid
flowchart LR
    A0((0)) --- A1((1)) --- A2((2<br/>front)) --- A3((3)) --- A4((4)) --- A5((5<br/>rear)) --- A6((6))
    A6 --- A0
...
> *   **Поза курсом:** у багатопотоковому програмуванні буває підхід, коли **результат не гарантований одразу** — він з'являється **асинхронно**; це окрема тема, ми тут розглядаємо лише синхронну чергу. Для неї якраз і підходять `bool` + `out`-параметр або **`std::optional`**: «значення є» або «немає» **зараз**, без очікування іншого потоку.
...
| Алгоритм (Операція) | Швидкість |
| --- | --- |
| `enqueue` / `dequeue` | $O(1)$ (істинно) |
| Послідовна обробка | $O(1)$ на елемент + кеш-локальність |
| Використання пам'яті | 100% без зсувів |
