/** Логіка покрокової візуалізації для Незв'язаного Графа */
...
function generateDisconnectedSteps(graph) {
  ...
  for (let v = 0; v < n; v++) {
    ...
    while (q.length > 0) {
      ...
      const neighbors = DISCONNECTED_ADJ[cur] || [];
...
GraphViz.init({
  title: "Компоненти зв'язності (Незв'язаний граф)",
  subtitle: "покроковий BFS обхід (лекція 22)",
  source: GRAPH_SOURCE,
  graph: DISCONNECTED_GRAPH,
  generateSteps: generateDisconnectedSteps
});
