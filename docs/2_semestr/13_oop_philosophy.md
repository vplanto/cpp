...
Згадайте Лекцію 1 ("Рівні абстракції").
...
│  📦 Data Members (State)            │
│                                     │
│  STATIC (Class-level, shared):      │  ← New: PDF Page 354
│     • static nextAccountId: int     │     (One copy for ALL objects)
│                                     │
│  INSTANCE (Object-level):           │  ← New: PDF Page 354
│     - balance: double               │     (Each object has own copy)
│     - owner: std::string            │
│     - accountNumber: int            │
├─────────────────────────────────────┤
...
```cpp
class Circle {
    const double PI = 3.14;  // OK in C++11+
    double radius;
    
public:
    Circle(double r) {
        PI = 3.14159;  // error: assignment of read-only data-member 'Circle::PI'
        radius = r;
    }
};
```

**Правило:** Завжди використовуйте initializer list для:
- Const членів
- Reference членів  
- Членів, які не мають default constructor
- Коли важлива performance (уникнути подвійної ініціалізації)
