...
2. Чому `operator<<` реалізується як `friend`-функція, а не як метод класу `Matrix`?
...
2. Лівим операндом `<<` є `std::ostream` (тобто `cout`). Метод класу `Matrix` міг би бути правим операндом, але синтаксис `cout << m` вимагає, щоб `cout` був зліва — а ми не можемо додати метод до `std::ostream`. `friend`-функція не є методом жодного класу, тому приймає обидва аргументи явно.
