...
> **💡 Розбираємо алгоритми STL:**
> 1. **Оператор `::`** — це **global namespace resolution operator**. Він вказує компілятору шукати функцію (`ispunct`, `tolower`) у глобальному просторі імен (C-style), щоб уникнути конфліктів.
> 2. **`std::remove_if`** — не видаляє елементи фізично! Він лише зсуває ті, що підходять під умову (`::ispunct`), у кінець контейнера і повертає ітератор на початок цього "сміття". Тому ми викликаємо `word.erase(...)`, щоб реально очистити пам'ять (це називається **Erase-Remove idiom**).
> 3. **`std::transform`** — проходить по всьому діапазону (`begin` до `end`) і застосовує до кожного елемента функцію (`::tolower`), зберігаючи результат у той самий рядок.
> 4. **Функції як параметри** — зверніть увагу, що ми передаємо `::ispunct` та `::tolower` як аргументи. В C++ ми можемо передавати назву функції (або її адресу) в іншу функцію (алгоритм), щоб той міг викликати її "всередині" себе для кожного елемента.
> 5. **`auto`** — ключове слово, яке дозволяє компілятору автоматично визначити тип змінної на основі значення, яким вона ініціалізується. Тут `tokenize` повертає `std::vector<std::string>`, тому `words` автоматично отримає саме цей тип.
